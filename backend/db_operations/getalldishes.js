...
const getdishes = (conn_pool)=>{return new Promise((resolve, reject)=>{
    //console.log(`select * from dishes where resteraunt_name = "${resto_name}"and zipcode ="${zipcode}";`)
    x=conn_pool.query(`select * from dishes;`,  (error, results)=>{
...
async function get_dishes () {
    ...
    try{
        ...
        const ins_res=await getdishes(conn_pool)
...
async function getdishes_resto(){
    let x= await get_dishes()
